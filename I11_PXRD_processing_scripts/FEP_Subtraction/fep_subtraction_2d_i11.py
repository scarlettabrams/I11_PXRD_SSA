...
class FrameMetricConfig:
    ...
    # Calibrated beam centre, in pixels, as (row, col) i.e. (cy, cx).
    # If None, falls back to the geometric frame midpoint (rows/2, cols/2)
    # and logs a warning - that fallback is NOT calibrated and should only
    # be used for quick exploration, never for real classification/subtraction.
    # Populate this from a validated .poni file via beam_center_px_from_poni().
    beam_center_px: Optional[tuple[float, float]] = None
...
def beam_center_px_from_poni(poni_path: Union[str, Path]) -> tuple[float, float]:
    """
    Read a pyFAI .poni file and return the calibrated beam centre in
    pixels, as (cy, cx) i.e. (row, col) — matching the (rows, cols)
    convention used everywhere else in this module.

    pyFAI's Poni1/Poni2 are in metres along the detector's axis-1
    (row/Y) and axis-2 (col/X) respectively (see the .poni file's own
    "Nota" header comment). Dividing by pixel size gives pixel units.

    Parameters
    ----------
    poni_path :
        Path to a validated .poni file, e.g. calib_50mm.poni.

    Returns
    -------
    (cy, cx) : tuple[float, float]
        Beam centre in pixels, row then column.

    Examples
    --------
    >>> cy, cx = beam_center_px_from_poni("calib_50mm.poni")
    >>> cfg = FrameMetricConfig(beam_center_px=(cy, cx))
    """
    ...
    with open(poni_path, "r") as f:
        for line in f:
            ...
            if line.startswith("Poni1:"):
                poni1 = float(line.split(":", 1)[1].strip())
            elif line.startswith("Poni2:"):
                poni2 = float(line.split(":", 1)[1].strip())
            elif line.startswith("Detector_config:"):
                import json
                cfg_json = json.loads(line.split(":", 1)[1].strip())
                pixel1 = float(cfg_json["pixel1"])
                pixel2 = float(cfg_json["pixel2"])

    if poni1 is None or poni2 is None or pixel1 is None or pixel2 is None:
        raise ValueError(
            f"Could not parse Poni1/Poni2/pixel size from {poni_path}. "
            "Check the file is a valid pyFAI .poni file."
        )

    cy = poni1 / pixel1
    cx = poni2 / pixel2
    logger.info(
        "Beam centre from %s: row(cy)=%.2fpx  col(cx)=%.2fpx",
        poni_path.name, cy, cx,
    )
    return cy, cx


def _resolve_beam_center(
    rows: int,
    cols: int,
    beam_center_px: Optional[tuple[float, float]],
) -> tuple[float, float]:
    """
    Return (cy, cx) to use for radial-profile geometry.

    If a calibrated beam_center_px is supplied, use it. Otherwise fall
    back to the geometric frame midpoint and warn loudly — this
    fallback does NOT reflect the real (off-centre) beam position
    confirmed by calibration, and should not be trusted for anything
    beyond quick exploration.
    """
    if beam_center_px is not None:
        return float(beam_center_px[0]), float(beam_center_px[1])
    warnings.warn(
        "No calibrated beam_center_px supplied - falling back to the "
        "geometric frame midpoint. This is NOT the real beam centre "
        "(calibration shows it is off-centre) and will bias radial "
        "profiles, FEP ring detection, and spot_density. Pass "
        "beam_center_px (e.g. from beam_center_px_from_poni()) via "
        "FrameMetricConfig before trusting classification/subtraction results.",
...
def evaluate_frame_with_status(
    ...
    rows, cols = arr.shape
    cy, cx = _resolve_beam_center(rows, cols, cfg.beam_center_px)
    ...
    fep_ring_r = int(cfg.fixed_fep_ring_r) if cfg.fixed_fep_ring_r is not None else None
    if fep_ring_r is None and not cfg.detect_fep_ring:
        raise ValueError(
            "No fixed_fep_ring_r supplied and detect_fep_ring=False - "
            "nothing to use as the ring radius. Either set "
            "detect_fep_ring=True or provide fixed_fep_ring_r explicitly "
            "(re-derive it at the corrected beam centre first, do not "
            "reuse the old r~255px value from the uncalibrated geometry)."
        )
    ...
    metrics = compute_frame_metrics(
        arr, filepath, frame_index=frame_index, mask=None,
        beam_center_px=cfg.beam_center_px,
    )
...
def compute_frame_metrics(
    ...
    beam_center_px :
        Calibrated (cy, cx) beam centre in pixels, from
        beam_center_px_from_poni(). If None, falls back to the
        geometric frame midpoint and logs a warning - only use that
        fallback for quick exploration, never for real results.
    ...
    # --- Radial geometry -----------------------------------------------------
    # Build r_map from the calibrated beam centre (from a validated .poni
    # file). Falls back to the geometric frame midpoint with a warning if
    # no calibration is supplied - see _resolve_beam_center().
    # NOTE: the r~255px FEP ring position quoted throughout this function's
    # comments was derived under the OLD uncalibrated (midpoint) geometry.
    # Re-check the ring position at the corrected centre before trusting
    # fep_annulus / detection_mask radii below on newly-calibrated data.
    cy, cx = _resolve_beam_center(rows, cols, beam_center_px)
    y_idx, x_idx = np.ogrid[:rows, :cols]
    r_map = np.sqrt((x_idx - cx) ** 2 + (y_idx - cy) ** 2).astype(np.int32)
    r_max = int(r_map.max())
...
def subtract_fep_2d(
    ...
    *,
    ...
    beam_center_px :
        Calibrated (cy, cx) beam centre in pixels, from
        beam_center_px_from_poni(). Only affects the provenance
        crystal_score/auto_class recorded in the returned
        SubtractionResult - the pixel-wise subtraction itself is
    ...
    metrics = compute_frame_metrics(
        raw, data_filepath, frame_index, beam_center_px=beam_center_px
    )
