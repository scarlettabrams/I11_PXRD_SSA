...
DEFAULTS = {
    ...
    "output_dir": "",
    "frame_index": 0,
    # ── production FEP correction (applied per frame in Inline/Batch) ──
    "fep_prod_enabled": True,
    "fep_prod_method": "subtract",
    "fep_prod_ref_path": "",
    "fep_prod_ref_data": None,     # dict: x, y, kind — loaded once, reused every frame
    "fep_prod_scale_min": 2.0,
    "fep_prod_scale_max": 4.0,
    "fep_prod_floor_frac": 0.02,
    "fep_prod_clip_negative": True,
}
...
def render_fep_ab_comparison(
    ...
    ab_half_window, ab_clip_negative, floor_frac=0.02,
):
    """
    Runs the full load -> combine (subtract or divide) -> MOR baseline pipeline
    for both references and renders the 3-stage plots + diagnostics table +
    ...
    with st.spinner(f"Loading, {method}, baselining..."):
        try:
            ...
            for label, x_r, y_r in [(ab_ref_a_label, x_a, y_a), (ab_ref_b_label, x_b, y_b)]:
                ...
                y_final, y_baseline = baseline_correct(x_s, y_comb, ab_half_window)

                diag_mask = (x_s >= ab_diag_min) & (x_s <= ab_diag_max)
                diag_std = float(y_final[diag_mask].std()) if diag_mask.sum() > 2 else float("nan")
                diag_mean = float(y_final[diag_mask].mean()) if diag_mask.sum() > 2 else float("nan")

                results_ab[label] = dict(
                    x=x_s, y_ref_used=y_ref_used, y_combined=y_comb,
                    y_baseline=y_baseline, y_final=y_final,
            ...
            # ── Stage 2: FEP-combined, pre-baseline ─────────────────
            stage2_verb = "division" if method == "divide" else "subtraction"
            st.markdown(f"##### Stage 2 — after FEP {stage2_verb} (pre-baseline)")
            st.caption(
                f"Dashed lines show the fitted MOR (morphological) baseline for each "
                f"reference, half_window={ab_half_window} — this is what gets subtracted "
                f"to produce Stage 3."
            )
            fig2 = go.Figure()
            for label, color in [(ab_ref_a_label, "#e74c3c"), (ab_ref_b_label, "#2980b9")]:
                ...
                fig2.add_trace(go.Scatter(x=r["x"], y=r["y_baseline"], mode="lines",
                                           name=f"{label} — MOR baseline fit",
            ...
            # ── Stage 3: final, MOR-baseline-corrected ───────────────
            st.markdown("##### Stage 3 — after MOR baseline correction (final)")
            st.caption("Final = Stage 2 pattern minus its dashed baseline fit above.")
            fig3 = go.Figure()
            for label, color in [(ab_ref_a_label, "#e74c3c"), (ab_ref_b_label, "#2980b9")]:
                r = results_ab[label]
                fig3.add_trace(go.Scatter(x=r["x"], y=r["y_final"], mode="lines",
                                           name=f"{label} — final",
                                           line=dict(color=color, width=1.6)))
            fig3.add_hline(y=0, line=dict(color="#aaa", width=1, dash="dot"))
            fig3.add_vrect(x0=ab_diag_min, x1=ab_diag_max, fillcolor="#8e44ad",
                           opacity=0.08, layer="below", line_width=0,
                           annotation_text="diagnostic region", annotation_position="top left",
                           annotation_font_size=10)
            fig3.update_layout(
                xaxis_title="2θ (°)", yaxis_title="Intensity",
                xaxis=dict(range=[rmin, rmax]), hovermode="x unified",
            ...
            for label in [ab_ref_a_label, ab_ref_b_label]:
                ...
                diag_rows.append({
                    ...
                    "Final max intensity": f"{r['y_final'].max():.1f}",
                    "Final peak 2θ (°)": f"{r['x'][int(np.argmax(r['y_final']))]:.3f}",
                })
            st.markdown("##### Comparison summary")
            st.dataframe(pd.DataFrame(diag_rows), use_container_width=True, hide_index=True)
            st.caption(
                "Lower diagnostic-region std/mean = cleaner, less-biased result in a region "
                "with no genuine sample signal. Also compare Stage 3 visually around any "
            ...
            # ── save both final patterns ─────────────────────────────
            if st.button("Save both final patterns as .xy", key=f"{key_prefix}_save"):
                out_dir = os.path.join(Path(ab_sample).parent, "06_fep_ab_comparison")
                ...
                for label in [ab_ref_a_label, ab_ref_b_label]:
                    ...
                    save_xy(out_xy, r["x"], r["y_final"],
                            header=f"2theta(deg) Intensity_FEP{method}_baselined")
...
def apply_fep_prod_correction(x, y):
    """
    Apply the sidebar-configured production FEP correction (if enabled and a
    reference is loaded) to a raw integrated pattern. Used by the Inline loop
    and Batch process tabs, between integration and MOR baselining.

    Returns
    -------
    y_out    : corrected pattern (or the original y, unchanged, if not applied)
    factor   : scale/renormalisation factor, or None if not applied
    applied  : bool
    """
    if not st.session_state.fep_prod_enabled or st.session_state.fep_prod_ref_data is None:
        return y, None, False
    ref = st.session_state.fep_prod_ref_data
    _, _, y_out, factor = combine_fep_1d(
        x, y, ref["x"], ref["y"],
        st.session_state.fep_prod_scale_min, st.session_state.fep_prod_scale_max,
        method=st.session_state.fep_prod_method,
        clip_negative=st.session_state.fep_prod_clip_negative,
        floor_frac=st.session_state.fep_prod_floor_frac,
    )
    return y_out, factor, True
...
with st.sidebar:
    ...
    st.markdown("### FEP correction (production)")
    st.caption(
        "Applied per frame in Inline loop / Batch process, between integration and "
        "MOR baselining. Set once per run — determine the reference/method first using "
        "the FEP subtraction → A/B comparison tool."
    )

    st.session_state.fep_prod_enabled = st.checkbox(
        "Enable FEP correction", value=st.session_state.fep_prod_enabled,
        help="Uncheck to skip FEP correction entirely and go straight integrate → baseline "
             "(the old behaviour).",
    )

    if st.session_state.fep_prod_enabled:
        fep_prod_method_choice = st.radio(
            "Method", options=["Subtract", "Divide"],
            index=0 if st.session_state.fep_prod_method == "subtract" else 1,
            horizontal=True,
            help="Subtract is the physically motivated default. Divide is test-only — "
                 "see the reasoning in the FEP subtraction tab.",
        )
        st.session_state.fep_prod_method = fep_prod_method_choice.lower()

        col_pf, col_pfb = st.columns([3, 1])
        with col_pf:
            fep_prod_ref_input = st.text_input(
                "Reference file (frame or .xy)",
                value=st.session_state.get("_browse_fep_prod_ref", st.session_state.fep_prod_ref_path),
                placeholder=r"E:/calib/Water_tri_ref.nxs  or  .xy",
                key="fep_prod_ref_input",
            )
        with col_pfb:
            st.markdown("<br>", unsafe_allow_html=True)
            browse_button("Browse", "file", "_browse_fep_prod_ref", "Select production FEP reference")
        if st.session_state.get("_browse_fep_prod_ref"):
            fep_prod_ref_input = st.session_state["_browse_fep_prod_ref"]

        if st.button("Load reference", use_container_width=True,
                     disabled=not fep_prod_ref_input):
            try:
                x_r, y_r, kind_r = load_1d_pattern_any(
                    fep_prod_ref_input, st.session_state.ai, st.session_state.mask,
                    st.session_state.npt, st.session_state.radial_min, st.session_state.radial_max,
                )
                st.session_state.fep_prod_ref_path = fep_prod_ref_input
                st.session_state.fep_prod_ref_data = {"x": x_r, "y": y_r, "kind": kind_r}
                st.success(f"Reference loaded ({kind_r})")
            except Exception as e:
                st.error(f"Could not load reference: {e}")

        if st.session_state.fep_prod_ref_data is not None:
            st.markdown(
                f'<p class="status-ok">● Reference ready</p>', unsafe_allow_html=True
            )
            st.caption(f"`{Path(st.session_state.fep_prod_ref_path).name}` "
                       f"({st.session_state.fep_prod_ref_data['kind']})")
        else:
            st.markdown('<p class="status-warn">● No reference loaded yet</p>', unsafe_allow_html=True)

        col_ps1, col_ps2 = st.columns(2)
        with col_ps1:
            st.session_state.fep_prod_scale_min = st.number_input(
                "Scale region min (°)", value=st.session_state.fep_prod_scale_min,
                min_value=0.0, max_value=10.0, step=0.5,
            )
        with col_ps2:
            st.session_state.fep_prod_scale_max = st.number_input(
                "Scale region max (°)", value=st.session_state.fep_prod_scale_max,
                min_value=0.0, max_value=15.0, step=0.5,
            )

        if st.session_state.fep_prod_method == "divide":
            st.session_state.fep_prod_floor_frac = st.slider(
                "Reference floor (fraction of ref max)",
                min_value=0.005, max_value=0.20,
                value=st.session_state.fep_prod_floor_frac, step=0.005,
            )

        st.session_state.fep_prod_clip_negative = st.checkbox(
            "Clip negative values to zero after combine step",
            value=st.session_state.fep_prod_clip_negative,
        )

        if st.session_state.fep_prod_ref_data is None:
            st.warning("Load a reference above — until then, frames process without FEP correction.")
...
tab_calib, tab_inline, tab_batch, tab_fep, tab_viewer, tab_status = st.tabs([
    "🎯  Calibration",
    "▶  Inline loop",
    "⚡  Batch process",
    "🧪  FEP subtraction",
...
with tab_inline:
    ...
    if st.session_state.fep_prod_enabled and st.session_state.fep_prod_ref_data is not None:
        st.caption(
            f"🧪 FEP correction: **{st.session_state.fep_prod_method}** using "
            f"`{Path(st.session_state.fep_prod_ref_path).name}` "
            f"(set in sidebar) — applied before baselining."
        )
    elif st.session_state.fep_prod_enabled:
        st.info("FEP correction is enabled but no reference is loaded yet — "
                "set one in the sidebar, or frames will process without it.")
    else:
        st.caption("🧪 FEP correction: **off** (enable in sidebar).")
            ...
            if process_btn:
                with st.spinner(f"Processing {frame_name}..."):
                    try:
                        ...
                        y_fep, fep_factor, fep_applied = apply_fep_prod_correction(x, y)
                        corrected, baseline = baseline_correct(
                            x, y_fep, st.session_state.half_window
                        )
                        hw = st.session_state.half_window
                        fep_tag = f"_fep{st.session_state.fep_prod_method}" if fep_applied else ""
                        out_stem = f"{frame_name}{fep_tag}_hw{hw}_baselined"
                        ...
                        st.success(
                            f"✓ Saved: {os.path.basename(xy_path)}"
                            + (f"  ·  FEP {st.session_state.fep_prod_method} factor={fep_factor:.4f}"
                               if fep_applied else "  ·  FEP correction OFF")
                        )
                        ...
                        if fep_applied:
                            fig_p.add_trace(go.Scatter(
                                x=x, y=y_fep, mode="lines",
                                name=f"FEP {st.session_state.fep_prod_method}ed (pre-baseline)",
                        ...
                        fig_p.update_layout(
                            title=f"{frame_name} — "
                                  + ("FEP corrected + " if fep_applied else "")
                                  + "baseline corrected",
                            xaxis_title="2θ (°)",
...
with tab_batch:
    ...
    if st.session_state.fep_prod_enabled and st.session_state.fep_prod_ref_data is not None:
        st.caption(
            f"🧪 FEP correction: **{st.session_state.fep_prod_method}** using "
            f"`{Path(st.session_state.fep_prod_ref_path).name}` "
            f"(set in sidebar) — applied before baselining, every frame."
        )
    elif st.session_state.fep_prod_enabled:
        st.info("FEP correction is enabled but no reference is loaded yet — "
                "set one in the sidebar, or frames will process without it.")
            ...
            if not target:
                ...
                for i, filepath in enumerate(target):
                    ...
                    try:
                        ...
                        save_xy(xy_path, x, corrected)
                        results.append({
                            "file": frame_name, "status": "✓ OK",
                            "max_intensity": f"{corrected.max():.1f}",
                            "fep": (f"{st.session_state.fep_prod_method} "
                                    f"(×{fep_factor:.3f})") if fep_applied else "off",
                        })
...
with tab_fep:
    ...
    st.caption(
        ...
        "1D post-integration (production) and 1D reference A/B comparison are available now; "
        "2D pre-integration is future work."
    )
    ...
    fep_method = st.radio(
        "Subtraction method",
        options=[
            "1D — post-integration (available)",
            "1D — reference A/B comparison (available)",
            "2D — pre-integration (coming soon)",
        ],
        horizontal=True,
        help=(
            "1D (production): subtract a single scaled FEP reference pattern from your "
            "merged/baselined 1D .xy file — order is integrate → baseline → subtract. "
            "1D (A/B comparison): compare two candidate reference patterns (e.g. raw FEP "
            "tube vs tri-segmented reference) on one sample — order is integrate → subtract "
            "→ baseline, so the scaling estimate isn't distorted by an already-baselined "
            "pattern. 2D: subtract FEP in 2D before azimuthal integration (future)."
        ),
    )
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # 1D A/B REFERENCE COMPARISON
    # ══════════════════════════════════════════════════════════════════════════
    if fep_method.startswith("1D — reference A/B"):
        st.markdown("#### Reference A/B comparison — raw FEP-ref vs tri-segmented ref")
        st.caption(
            "Runs the same sample through two candidate FEP references and shows all three "
            "pipeline stages side by side: raw integrated → FEP-combined (pre-baseline) → "
            "MOR-baselined. Accepts either a raw .nxs/.hdf frame (integrated on the fly using "
            "the loaded calibration — multi-frame files are averaged) or a pre-integrated .xy "
            "file for the sample and for each reference. Pick Subtract or Divide below — each "
            "runs and displays independently so results aren't stacked in one long scroll."
        )
        ...
        st.markdown(
            ...
            "in the *final* baselined pattern; used to score residual noise/bias for each reference"
        )
        ...
        col_hw, col_clip = st.columns(2)
        with col_hw:
            ab_half_window = st.number_input(
                "MOR baseline half_window (applied after the combine step)",
                min_value=2, max_value=30, value=st.session_state.half_window,
                key="ab_half_window",
                help="Same pybaselines morphological (MOR) baseline used elsewhere in the app.",
            )
        with col_clip:
        ...
        st.caption(
            "Sample, references, and the regions/half_window above are shared by both sub-tabs "
        ...
        with ab_tab_sub:
            ...
            render_fep_ab_comparison(
                ...
                ab_half_window=ab_half_window, ab_clip_negative=ab_clip_negative,
            )
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # 1D METHOD — single reference (production)
    # ══════════════════════════════════════════════════════════════════════════
    elif fep_method.startswith("1D — post-integration"):
